...
local TbacGroup
local OldTbacGroup
local TbacGroupName = 'Vendre Moonshine'
local Wprompt
local IsTargetting
local OldEnt
local hasItem = false
...
Citizen.CreateThread(function()
    while true do
        ...
        TriggerServerEvent("SellToNPC:CheckHasItems")
    end
end)

RegisterNetEvent("SellToNPC:HasItems")
AddEventHandler("SellToNPC:HasItems", function(item)
    hasItem = item
end)
                                ...
                                if OldEnt ~= entity then
                                    ...
                                    if r == 2 then
                                        ...
                                        if D <= 2.5 and not DeadBruh or KnockedOut then 
                                            ...
                                            TriggerEvent("vorp:TipBottom", 'La personne est trop loin !', 6000) 
                                        end
                                        ClearPedTasks(entity)
...
AddEventHandler("Witness:ToggleNotification2", function(coords)
	TriggerEvent("vorp:TipBottom", 'Une personne vend de la moonshine', 5000)
	local blip = Citizen.InvokeNative(0x45f13b7e0a15c880, -1282792512, coords.x, coords.y, coords.z, 25.0)
	Wait(15000)--Time till notify blips dispears, 1 min
	RemoveBlip(blip)
end)